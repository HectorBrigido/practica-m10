document.addEventListener('DOMContentLoaded', () => {
    ...
    const mainContent = document.querySelector('main'); // Contenedor principal para la accesibilidad del modal
    ...
    // Asegurar que los mensajes de error sean leídos por lectores de pantalla.
    errorMessageContainer.setAttribute('aria-live', 'polite');

    // Preparar el modal para ser accesible (ARIA attributes).
    historyModal.setAttribute('aria-hidden', 'true');
    historyModal.setAttribute('aria-modal', 'true');
    historyModal.setAttribute('role', 'dialog');
    historyModal.setAttribute('aria-labelledby', 'modal-title');
    ...
    let lastFocusedElement; // Para devolver el foco al cerrar el modal (accesibilidad)
    ...
    const updateStatusOverview = () => {
        // MEJORA: Usar reduce para contar todos los estados.
    ...
    const handleTableActions = (event) => {
        ...
        if (action === 'history') {
            const guide = guides.find(g => g.id === guideId);
            lastFocusedElement = target; // Guardar el elemento que activó el modal
            if (guide) {
    ...
    const showHistoryModal = (guide) => {
        ...
        if (mainContent) mainContent.setAttribute('aria-hidden', 'true'); // Ocultar el resto del contenido a lectores de pantalla

        document.addEventListener('keydown', handleModalKeydown);
        modalCloseBtn.focus(); // Mover el foco al botón de cerrar para accesibilidad
    };
    ...
    const hideHistoryModal = () => {
        ...
        historyModal.setAttribute('aria-hidden', 'true');
        if (mainContent) mainContent.setAttribute('aria-hidden', 'false'); // Volver a mostrar el contenido
    ...
    /**
     * Maneja el cierre del modal con la tecla Escape para mejorar la accesibilidad.
     */
